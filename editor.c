...
#define EDITOR_QUIT_TIMES 3 /**how many extra times to hit quick for it to exit without saving**/
...
void editorDrawStatusBar(struct abuf *ab) {
	...
	int len = snprintf(status, sizeof(status), "%.20s - %d lines %s",
			...
			E.dirty ? "(modified)" : "");
	int rlen = snprintf(status, sizeof(rstatus), "%s | %d/%d",
   ...
   case CTRL_KEY('q'):
      if (E.dirty && quit_times > 0) {
	      editorSetStatusMessage("Warning!! File has unsaved changes."
			      "Prss CTRL-Q %d more times to quit.", quit_times);
	      quit_times--;
	      return;
      }
      write(STDOUT_FILENO, "\x1b[2J", 4);
